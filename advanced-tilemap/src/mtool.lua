...
function mtool.create_mesh_from_tilemap(mesh_url, options)
	...
	local ambient = options.ambient or vmath.vector4(1, 1, 1, 1)
	local dummy = vmath.vector3()
		...
		for x = bx, bx2 do
			...
			if tileno and tileno>0 then
				...
				if options.lights then
					for i = 1, #options.lights do
						local pos = options.lights[i].position
						local cl = options.lights[i].color
						local power = options.lights[i].power

						local dx = dummy.x - pos.x
						local dy = dummy.y - pos.y
				...
				for edge = 1, edges do
					...
					if not skip then
						...
						for i = 0, 5 do
							...
							positions[index + 1] = dummy.x + (v.x - 1) * scale
							positions[index + 2] = dummy.y + (v.y - 1) * scale
							positions[index + 3] = dummy.z + v.z * scale 
